...
io.on("connection", async (socket) => {
	...
	try {
		...
		socket.on("UpdateCurrentSessionDiff", async (diff) => {
			...
			const newDataJson = updateSessionValue(currentDataJson, diff);
			...
			try {
				await updateSession(currentDataJson, newDataJson, socket);
			} catch (error) {
