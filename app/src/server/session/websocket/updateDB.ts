...
const updateDatabase = async (code: string, newData: SessionValue, clients: Map<string, ws>) => {
    // 既存のセッションデータを取得
    const existingDataJson = await sessionDB.get(code);
    const existingData: SessionValue = existingDataJson ? JSON.parse(existingDataJson) : null;
