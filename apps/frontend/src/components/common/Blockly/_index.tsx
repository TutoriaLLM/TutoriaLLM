...
const Editor = forwardRef<
	HTMLDivElement,
	{
		...
		setWorkspace: Dispatch<SetStateAction<SessionValue["workspace"] | null>>;
	}
>(({ menuOpen, language, currentWorkspace, setWorkspace }, ref) => {
	...
	useEffect(() => {
		...
		const saveWorkspace = (event: Blockly.Events.Abstract) => {
			...
			const isCommentChange =
				...
				event.type === Blockly.Events.COMMENT_RESIZE ||
			...
			try {
				if (isBlockChange || isCommentChange || isVarChange) {
					...
					setWorkspace((prev) => {
						console.log("Workspace changed", prev, newWorkspace);
						if (
						...
						console.log("Workspace not changed");
						return prev;
					});
